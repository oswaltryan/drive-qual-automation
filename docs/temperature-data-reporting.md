...
- `drive_qual.core.temperature` owns artifact paths, CSV parsing, and updates to
  the report JSON `temperature -> <DUT> -> performance` contract.
...
```powershell
uv run drive-qual-temperature --part-number 69-420 --dut "Padlock DT" --csv .\matched.csv --chart .\temperature.png
```
